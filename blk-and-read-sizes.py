...
BPF_HISTOGRAM(user_reads, dev_key_t);
BPF_HISTOGRAM(user_writes, dev_key_t);
...
static int user_operation(struct kiocb *iocb, struct iov_iter *iov, int write)
{
    dev_t device = iocb->ki_filp->f_mapping->host->i_sb->s_bdev->bd_disk->part0.__dev.devt;
    int major = MAJOR(device);
    int minor = MINOR(device);
    u64 count = iov->count;
    ...
    dev_key_t key = {
        .dev = device,
        .slot = bpf_log2l(count),
    };

    if (write)
        user_writes.increment(key);
    else
        user_reads.increment(key);
    return 0;
}

int trace_generic_file_read_iter(struct pt_regs *ctx, struct kiocb *iocb,
				 struct iov_iter *iov)
{
    return user_operation(iocb, iov, 0);
}

int trace_generic_file_write_iter(struct pt_regs *ctx, struct kiocb *iocb,
				  struct iov_iter *iov)
{
    return user_operation(iocb, iov, 1);
}
"""
...
b.attach_kprobe(event="generic_file_write_iter", fn_name="trace_generic_file_write_iter")

reads = b.get_table("reads")
writes = b.get_table("writes")
discards= b.get_table("discards")
user_reads = b.get_table("user_reads")
user_writes = b.get_table("user_writes")
...
user_reads.print_log2_hist("User Reads", "dev", section_print_fn=print_device)
user_writes.print_log2_hist("User Writes", "dev", section_print_fn=print_device)
